...
%         Nthr = max(20*maxNumCompThreads,round(d*T/2^24));
        Nthr = min(round(d*T/2^24),round(d/1e3));
        ...
        for nthr = 1:Nthr     
            ...
            for px = 1:siz_row(nthr)
                ...
                Cf_temp{px} = Cf_ds(ind{px},:);
            end

            parfor px = 1:siz_row(nthr)
                thisY = Ytemp(px,:);
                thisCf = Cf_temp{px};
                fn = ~isnan(thisY);       % identify missing data
%                 ind = find(IND_temp(px,:));
%                 if ~isempty(ind);
%                     ind2 = [ind,K+(1:size(f,1))];
                [~, ~, a, ~] = lars_regression_noise(thisY(fn)', thisCf(:,fn)', 1, sn_temp(px)^2*T);
                a_sparse = sparse(1,ind{px},double(a'));
